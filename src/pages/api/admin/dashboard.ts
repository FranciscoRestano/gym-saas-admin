...
export const GET: APIRoute = async ({ cookies }) => {
  ...
  const [clientesRes, pagosMesRes, planesRes, pagosAllRes, profesoresRes] = await Promise.all([
    supabase
      .from('clientes')
      .select('id, nombre, dni, plan_id, profesor_id')
      .eq('gimnasio_id', GIMNASIO_ID),
    supabase
      .from('pagos')
      .select('monto_pagado')
      .eq('gimnasio_id', GIMNASIO_ID)
      .gte('fecha_pago', primerDiaMesActual)
      .lt('fecha_pago', primerDiaMesSiguiente),
    supabase
      .from('planes')
    ...
    supabase
      .from('profesores')
      .select('id, nombre, horas_semanales, pago_por_hora, comision_por_alumno')
      .eq('gimnasio_id', GIMNASIO_ID),
  ]);

  const errors = [clientesRes, pagosMesRes, planesRes, pagosAllRes, profesoresRes]
    .map((r) => r.error)
    .filter(Boolean);

  if (errors.length > 0) {
    return new Response(JSON.stringify({ error: errors.map((e) => e!.message).join('; ') }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
