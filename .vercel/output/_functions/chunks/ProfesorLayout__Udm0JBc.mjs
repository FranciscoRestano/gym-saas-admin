...
/* empty css                 */
//#region src/layouts/ProfesorLayout.astro
createAstro("https://astro.build");
var $$ProfesorLayout = createComponent(($$result, $$props, $$slots) => {
	const Astro = $$result.createAstro($$props, $$slots);
	Astro.self = $$ProfesorLayout;
	...
	const navIcons = {
		alumnos: `<path d="M9 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" /><path d="M3 19v-1c0-2.21 2.686-4 6-4s6 1.79 6 4v1" /><path d="M16 8a2.5 2.5 0 1 0 0-5" /><path d="M19 19v-1c0-1.657-1.79-3-4-3" />`,
	...
	return renderTemplate`<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${title} · Profesor</title>${renderHead($$result)}</head><body class="bg-zinc-50 text-zinc-900 antialiased"><div class="min-h-screen lg:flex"><!-- Barra superior móvil --><header class="sticky top-0 z-30 flex items-center justify-between border-b border-zinc-200 bg-white px-4 py-3 lg:hidden"><span class="flex items-center gap-2 font-semibold text-zinc-900"><span class="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500 text-white"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5">${unescapeHTML(brandMark)}</svg></span>Mi Panel</span><button id="sidebar-toggle" type="button" aria-label="Abrir menú" class="rounded-md p-2 text-zinc-600 hover:bg-zinc-100"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" class="h-6 w-6"><path d="M4 6h16M4 12h16M4 18h16"></path></svg></button></header><div id="sidebar-overlay" class="fixed inset-0 z-30 hidden bg-zinc-900/50 lg:hidden"></div><!-- Sidebar --><aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col bg-zinc-900 transition-transform duration-200 ease-out lg:static lg:z-auto lg:w-64 lg:translate-x-0"><div class="flex h-16 shrink-0 items-center gap-2 border-b border-white/10 px-6"><span class="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500 text-white"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5">${unescapeHTML(brandMark)}</svg></span><span class="text-lg font-semibold text-white">Mi Panel</span></div><nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4" aria-label="Navegación principal">${navItems.map((item) => {
	...
	})}</nav><div class="border-t border-white/10 px-3 py-4"><button id="btn-cerrar-sesion" type="button" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-zinc-400 hover:bg-white/5 hover:text-red-400 transition-colors"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5 shrink-0"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg>Cerrar Sesión</button></div></aside><!-- Contenido principal --><main class="flex-1 px-4 py-6 sm:px-6 lg:px-10 lg:py-10"><div class="mx-auto max-w-6xl space-y-6">${renderSlot($$result, $$slots["default"])}</div></main></div>${renderScript($$result, "/home/francisco/Escritorio/Proyectos/gym-saas-admin/src/layouts/ProfesorLayout.astro?astro&type=script&index=0&lang.ts")}</body></html>`;
}, "/home/francisco/Escritorio/Proyectos/gym-saas-admin/src/layouts/ProfesorLayout.astro", void 0);
//#endregion
export { $$ProfesorLayout as t };
